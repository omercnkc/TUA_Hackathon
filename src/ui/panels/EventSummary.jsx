...
export default function EventSummary({ report, events }) {
  ...
  return (
    <div style={{ marginTop: "16px", padding: "16px", border: "1px solid #ccc" }}>
      ...
      <div>Max Height: {report.maxHeight?.toFixed(2) ?? "0.00"} m</div>
      <div>Max Velocity: {report.maxVelocity?.toFixed(2) ?? "0.00"} m/s</div>
      <div>Total Flight Time: {report.flightTime?.toFixed(2) ?? "0.00"} s</div>
      <div>Impact Velocity: {report.impactVelocity?.toFixed(2) ?? "0.00"} m/s</div>
